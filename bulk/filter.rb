...
FILTER = File.expand_path(File.join(File.dirname(__FILE__), "pipette", "bin", "fast_filter_matching_vcfs.rb"))

ANNOTATE = File.expand_path(File.join(File.dirname(__FILE__), "pipette", "bin", "annotate_vcf.rb"))

ANNOTATE_CONFIG = File.expand_path(File.join(File.dirname(__FILE__), "variant_config.yml"))

LINKS = File.expand_path(File.join(File.dirname(__FILE__), "pipette", "bin", "add_igv_links.rb"))
...
def filter_vcfs vcf_files
  vcf_files.each do |vcf_file|
    ...
    Dir.chdir(dir_name) do
      ...
      command = "#{FILTER} #{vcf_basename} #{filter_file}"
      puts command
        system(command)
...
tsv_files = Dir.glob(File.join(starting_dir, "**", "*.collapse.txt"))
#add_links(tsv_files)

result_files = Dir.glob(File.join(starting_dir, "**", "*.links.xls"))
output_dir = File.join("results", "linked_files")
copy_results result_files, output_dir

output_dir = File.join("results", "summary_files")
result_files = Dir.glob(File.join(starting_dir, "**", "*.snpeff.summary.html"))
copy_results result_files, output_dir
